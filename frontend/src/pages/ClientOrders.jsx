...
const ClientOrders = () => {
  ...
  const sendWhatsAppInvoice = async (order) => {
    ...
    Swal.fire({
      ...
      text: 'Generating invoice image and redirecting to WhatsApp...',
    ...
    setTimeout(async () => {
      if (invoiceRef.current) {
        try {
          ...
          let invoiceUrl = order.invoiceImage || '';

          // Upload invoice image to Cloudinary / backend if order has an ID
          if (order._id) {
            try {
              ...
              if (data && (data.imageUrl || data.invoiceImage)) {
                invoiceUrl = data.imageUrl || data.invoiceImage;
              }
            } catch (upErr) {
              console.warn('Invoice image upload warning:', upErr);
            }
          }

          // Desktop clipboard copy for convenient pasting
          try {
          ...
          const message = `*INVOICE - SAPP CREATION*\n` +
            `━━━━━━━━━━━━━━━━━━━━\n` +
  ...
  const permanentClientPhones = new Set(clients.map(c => c.mobileNumber.replace(/\D/g, '')));
  ...
  const statusOptions = ['Pending', 'Processing', 'Printing', 'Ready To Dispatch', 'Delivered', 'Cancelled'];
      ...
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-4">
        ...
        <div className="d-flex flex-column align-items-stretch align-items-sm-end gap-2">
          <div className="d-flex gap-2 w-100 justify-content-sm-end">
            <Link to="/admin/orders" className="flex-grow-1 flex-sm-grow-0 text-decoration-none">
              <Button variant="outline-info" className="d-flex align-items-center justify-content-center text-nowrap shadow-sm w-100 h-100">
                Walk-in Orders
              </Button>
            </Link>
